...
// Find the FuncDecl that contains position 5706
let ip = 0;
while (ip < data.code.length) {
  ...
  if (tag === T.OP_FUNC_DECL) {
    ...
    if (ip <= 5706 && ip + dataCount > 5706) {
      console.log('FuncDecl at ip=' + ip + ' endAddr=' + endAddr + ' argc=' + argc + ' dataCount=' + dataCount);
      console.log('Function spans ip ' + ip + ' to ' + (ip + dataCount));
      if (5706 >= ip && 5706 < ip + dataCount) {
        console.log('Position 5706 is INSIDE this function');
      }
    }
