...
var Compiler = class {
  ...
  compileAST(stmts) {
    ...
    this.inFunction = false;
    this.breakLineCount = 0;
    const _pc0=Date.now(); const codeSize = this.precompileBlock(stmts, 0) + 1; console.error("PRE="+(Date.now()-_pc0)+"ms size="+codeSize);
    ...
    this.breakLineCount = 0;
    const _cc0=Date.now(); if (stmts.length > 0) this.compileBlock(context, stmts); console.error("COMP="+(Date.now()-_cc0)+"ms");
