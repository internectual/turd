...
import { FileLoader, Opcode, Ops, OpcodeTag, decompile } from './src/decompiler';
...
// Walk through the code stream as the decompiler would
let ip = 0;
let count = 0;
while (ip < data.code.length && count < 30) {
  ...
  if (!op) {
    ...
    if (data.identifierTable.has(ip)) {
      console.log('This position IS in identifier table, strIdx:', data.identifierTable.get(ip));
    }
    break;
  }
