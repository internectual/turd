import { Compiler } from './src/compiler/index';
import { FileLoader, Opcode, Ops, OpcodeTag } from './src/decompiler';
import * as fs from 'fs';

const ops = new Ops();
const T = OpcodeTag;

const code = fs.readFileSync('/home/methodown/t2-linux/console_start.cs', 'utf8');
const c = new Compiler('TGE10');
const b = c.compile(code);
const data = new FileLoader().load(new Uint8Array(b));

// Check identifier table around position 653
console.log('Identifier table entries 645-660:');
for (const [ip, strIdx] of data.identifierTable) {
  if (ip >= 645 && ip <= 660) {
    const entry = data.globalStringTable.get(strIdx);
    console.log('  ip=' + ip + ' strIdx=' + strIdx + ' string="' + (entry?.string || 'N/A') + '"');
  }
}

// Walk through instructions to find what should be at position 653
let ip = 0;
let count = 0;
while (ip < data.code.length && ip <= 660) {
  const val = data.code[ip];
  const op = Opcode.create(val, ops);
  if (!op) {
    console.log('\nINVALID at ip=' + ip + ' val=' + val);
    break;
  }
  
  const tag = op.tag;
  let dataCount = 0;
  if (tag === T.OP_FUNC_DECL) {
    const argc = data.code[ip + 6];
    dataCount = 6 + argc;
    if (ip >= 640 && ip <= 660) console.log('ip=' + ip + ' FuncDecl argc=' + argc + ' endAddr=' + data.code[ip + 5]);
  } else if (tag === T.OP_CREATE_OBJECT) {
    dataCount = 3;
    if (ip >= 640 && ip <= 660) console.log('ip=' + ip + ' CreateObject');
  } else if (tag === T.OP_ADD_OBJECT || tag === T.OP_END_OBJECT) {
    dataCount = 1;
  } else if (tag === T.OP_JMP || tag === T.OP_JMPIF || tag === T.OP_JMPIFF || 
             tag === T.OP_JMPIFNOT || tag === T.OP_JMPIFFNOT || 
             tag === T.OP_JMPIF_NP || tag === T.OP_JMPIFNOT_NP) {
    dataCount = 1;
    if (ip >= 640 && ip <= 660) console.log('ip=' + ip + ' Branch target=' + data.code[ip + 1]);
  } else if (tag === T.OP_CALLFUNC || tag === T.OP_CALLFUNC_RESOLVE) {
    dataCount = 3;
    if (ip >= 640 && ip <= 660) console.log('ip=' + ip + ' CallFunc');
  } else if (tag === T.OP_LOADIMMED_IDENT || tag === T.OP_LOADIMMED_STR || 
             tag === T.OP_TAG_TO_STR || tag === T.OP_SETCURVAR || 
             tag === T.OP_SETCURVAR_CREATE || tag === T.OP_SETCURFIELD ||
             tag === T.OP_ADVANCE_STR_APPENDCHAR || tag === T.OP_LOADIMMED_FLT ||
             tag === T.OP_LOADIMMED_UINT) {
    dataCount = 1;
    if (ip >= 640 && ip <= 660) console.log('ip=' + ip + ' tag=' + tag + ' val=' + val);
  } else {
    if (ip >= 640 && ip <= 660) console.log('ip=' + ip + ' tag=' + tag + ' val=' + val);
  }
  
  ip += 1 + dataCount;
  count++;
}
